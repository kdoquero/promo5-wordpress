...
add_action( 'edit_form_after_title', 'wplp_edit_form_after_title' );
...
function wplp_edit_form_after_title() {
echo '<h2>Description of Link:</h2>';
}
...
function wplp_subpage_options() {
	...
	function wplp_shortcode($atts){	
		...
		if (!isset($atts['display']) && get_option('wplpf_grid') != false) {
				$atts['display'] = esc_attr( get_option('wplpf_grid') );
		}
		
		if (!isset($atts['cols']) && get_option('wplpf_width') != false) {
